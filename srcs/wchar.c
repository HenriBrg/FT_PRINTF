...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/15 18:42:37 by hberger           #+#    #+#             */
/*   Updated: 2019/10/15 18:48:39 by hberger          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** La fonction get_size_wchar retourne la taille bytes/octets du wchar_t donné
*/

int		get_size_wchar(wchar_t wc)
{
	if (wc <= 127)
		return (1);
...
/*
** wchar_to_str() converti un wchar de taille
** octets (1 si ascii < 127, 2, 3 voire 4)
** en un string de taille octets + 1 (pour le '\0')
** La structure des bitwise a été récupéré sur le web,
** donc TODO : capter ce qu'ils font
*/
...
** Mais ce \O peut semblerai-t-il être placé sur l'octet 1, 2, 3 d'un wchar_t
*/
...
** 1 - On détermine la taille de la chaine de wchar_t avec wcstrlen()
** (code source de la fonction système wcslen)
** 2 - On alloue str avec la taille nécessaire pour accueilir chaque wchar_t
**     Exemple :  'a' en wchar_t possède une size de 1 (bytes, == octet)
** 					obtenue via get_size_wchar()
...
char	*strwchar_to_str(wchar_t *strwc)
{
	...
	while (++i < size_wc)
  		alloc_size_str += get_size_wchar(strwc[i]);
	if ((str = malloc(sizeof(char) * (alloc_size_str + 1))) == 0)
		return (0);
	str[0] = '\0';
	i = -1;
	while (++i < size_wc)
		str = ft_strjoin(str, wchar_to_str(strwc[i], get_size_wchar(strwc[i])));
	return (str);
}
...
void	convert_wchart_and_wstring(t_printf *tab, char c)
{
	...
	if (c == 'C')
	{
		...
		tab->output = wchar_to_str(wc, get_size_wchar(wc));
	}
	else if (c == 'S')
	{
